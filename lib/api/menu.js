 ...
 * Admin only (RLS enforces)
 */
export async function createMenuItem({ name, categoryId, price, prepTime, imageUrl = null, isVeg = false }) {
  const supabase = createClient();
 ...
 * Update menu item
 * Admin only (RLS enforces)
 * Can update: name, category, price, prep time, is_available, is_archived, imageUrl
 */
export async function updateMenuItem(itemId, { name, categoryId, price, prepTime, isAvailable, isArchived, imageUrl = null, isVeg = false }) {
  const supabase = createClient();
