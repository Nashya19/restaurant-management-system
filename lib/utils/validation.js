...
export const validatePhone = (phone) => {
  if (phone === null || phone === undefined || phone === '') return null; // optional
...
export const validateTableForm = ({ tableNumber, capacity, qrCodeUrl }) => {
  const errors = {};
  const tableNumberValue = parseInt(tableNumber, 10);
  const capacityValue = parseInt(capacity, 10);

  if (!tableNumber || Number.isNaN(tableNumberValue)) {
    errors.tableNumber = 'Table number is required';
  } else if (tableNumberValue < 1) {
    errors.tableNumber = 'Table number must be at least 1';
  } else if (tableNumberValue > 999) {
    errors.tableNumber = 'Table number cannot exceed 999';
  }

  if (!capacity || Number.isNaN(capacityValue)) {
 ...
 */
export const validateUserForm = ({ email, password, fullName, role, phone = '', isNewUser = true }) => {
  const errors = {};
  if (isNewUser) {
