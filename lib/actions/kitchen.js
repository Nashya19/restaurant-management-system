...
export async function adjustOrderItemStartedAtAction(orderItemId, itemStartedAt) {
  ...
  try {
    const dateObj = new Date(itemStartedAt);
    if (isNaN(dateObj.getTime())) {
      throw new Error('Invalid timestamp provided.');
    }
    
    if (dateObj.getTime() > Date.now()) {
      throw new Error('Cannot set started time to a future date.');
    }

    if (Date.now() - dateObj.getTime() > 24 * 60 * 60 * 1000) {
      throw new Error('Cannot set started time that is more than 24 hours ago.');
    }
