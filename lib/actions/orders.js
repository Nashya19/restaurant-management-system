...
export async function placeOrderAction(sessionId, items) {
  ...
  try {
    ...
    // 1b. Fetch menu item prep times for wait time calculation
    const menuItemIds = items.map(i => i.item.id);
    const { data: menuMeta } = await adminClient
      .from('menu_items')
      .select('id, prep_time_minutes')
      .in('id', menuItemIds);
    const prepByMenuId = {};
    (menuMeta || []).forEach(m => { prepByMenuId[m.id] = m.prep_time_minutes; });

    // 1c. Compute estimated wait from current queue
    const [queueResult, kitchenSettings] = await Promise.all([
      adminClient
        .from('order_items')
        .select('item_status, menu_items ( prep_time_minutes )')
        .in('item_status', ['pending', 'preparing'])
        .not('order_id', 'is', null),
      getKitchenSettingsAction()
    ]);
    const queuedItems = ((queueResult.data) || []).map(i => ({
      item_status: i.item_status,
      prep_time_minutes: i.menu_items?.prep_time_minutes
    }));
    const newOrderItemsForCalc = items.map(i => ({
      prep_time_minutes: prepByMenuId[i.item.id],
      quantity: i.qty
    }));
...
export async function temporarilyUnlockSessionAction(sessionId) {
  ...
  try {
    const unlockUntil = new Date(Date.now() + 60 * 1000).toISOString();
...
/**
 * Server Action: Save customer rating and custom comments for all orders in a session
 */
export async function submitRatingAction(sessionId, rating, comments = '') {
  const adminClient = createAdminClient();
  ...
  try {
    const { error } = await adminClient
      ...
      .update({
        rating,
        feedback_comment: comments.trim() || null
      })
      .eq('session_id', sessionId);
...
/**
 * Server Helper: Recalculate session total
 */
async function recalculateSessionTotal(adminClient, sessionId) {
  const { data: sessionOrders, error: fetchErr } = await adminClient
    .from('orders')
    .select(`
      id,
...
/**
 * Server Action: Update order item quantity (Admin/Staff only)
 */
export async function updateOrderItemQtyAction(orderItemId, newQty) {
  const adminClient = createAdminClient();
  try {
    const { data: itemData, error: itemErr } = await adminClient
      .from('order_items')
      .select('order_id, orders(session_id)')
      .eq('id', orderItemId)
      .single();
    if (itemErr || !itemData) throw new Error('Order item not found');
    const sessionId = itemData.orders.session_id;

    if (newQty <= 0) {
      const { error } = await adminClient.from('order_items').delete().eq('id', orderItemId);
      if (error) throw error;
    } else {
      const { error } = await adminClient
        .from('order_items')
    ...
    await recalculateSessionTotal(adminClient, sessionId);
    return { success: true };
  } catch (err) {
    throw new Error(err.message);
  }
...
/**
 * Server Action: Remove order item (Admin/Staff only)
 */
export async function removeOrderItemAction(orderItemId) {
  const adminClient = createAdminClient();
  try {
    const { data: itemData, error: itemErr } = await adminClient
      .from('order_items')
...
/**
 * Server Action: Delete entire order (Admin only)
 */
...
/**
 * Server Action: Add item to existing order (Admin/Staff only)
 */
export async function addOrderItemAction(orderId, menuItemId, quantity, priceAtOrder) {
  const adminClient = createAdminClient();
  try {
    const { data: orderData, error: orderErr } = await adminClient
      .from('orders')
...
/**
 * Server Action: Save order edits in a single transaction (Admin/Staff only)
 */
export async function saveOrderEditsAction(orderId, editedItems) {
  const adminClient = createAdminClient();
  try {
    const { data: orderData, error: orderErr } = await adminClient
      .from('orders')
    ...
    const { data: dbItems, error: dbErr } = await adminClient
      .from('order_items')
      .select('id')
      .eq('order_id', orderId);
    ...
    const editedItemIds = editedItems.filter(item => item.id).map(item => item.id);

    // 2. Delete items that were removed
    const idsToDelete = dbItemIds.filter(id => !editedItemIds.includes(id));
...
/**
 * Server Action: Create a Razorpay Order for a table session.
 * Fallback to mock order if API credentials are invalid/missing.
 */
export async function createRazorpayOrderAction(sessionId) {
  const adminClient = createAdminClient();
  const keyId = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || 'rzp_test_zenithRestKey';
  const keySecret = process.env.RAZORPAY_KEY_SECRET;

  try {
    // 1. Fetch running total from table session
    ...
    const amountInINR = sessionData.running_total || 0;
    const amountInPaise = Math.round(amountInINR * 100);

    // If keySecret is missing, skip the API call and return fallback
    if (!keySecret) {
      return {
        success: false,
        fallback: true,
    ...
    const res = await fetch('https://api.razorpay.com/v1/orders', {
      ...
      body: JSON.stringify({
        amount: amountInPaise,
        currency: 'INR',
        receipt: `receipt_${sessionId.substring(0, 8)}`
      })
    });

    if (!res.ok) {
      const errText = await res.text();
      console.warn('[Razorpay API] Order creation failed:', errText);
      return {
        success: false,
        fallback: true,
        keyId,
    ...
    console.error('[Razorpay Server Action] Error:', err);
    return {
      success: false,
      fallback: true,
      keyId,
